...
def main(HEIGHT, WIDTH, ENTRY, EXIT, OUTPUT_FILE, PERFECT, SEED=None, algo="Kru"):
    choice = 1
    anim = False
    color_walls = 0
    color_42 = 0
    ...
    while choice != 9:
        try:
            if choice == 1:
                maze = MazeGenerator(HEIGHT, WIDTH, ENTRY, EXIT, PERFECT, SEED)
                if WIDTH < 6 or WIDTH < 8:
                    mesage = "Cannot place '42' pattern without collision"
                else:
                    maze.p42()
                if algo is None or algo == "kru":
...
main(config["HEIGHT"], config["WIDTH"], config["ENTRY"], config["EXIT"],
     config["OUTPUT_FILE"], config["PERFECT"], config.get('seed', None), "kru")
